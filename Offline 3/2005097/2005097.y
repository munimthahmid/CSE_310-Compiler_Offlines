...
%type<symbol> start program unit var_declaration func_declaration func_definition type_specifier parameter_list compound_statement statements statement expression_statement expression logic_expression variable rel_expression simple_expression term unary_expression factor argument_list arguments declaration_list LCURL_BEGIN logic_expression_list
...
|type_specifier variable ASSIGNOP LCURL logic_expression_list RCURL SEMICOLON 
{
	$$=new SymbolInfo("type_specifier variable ASSIGNOP LCURL logic_expression_list RCURL SEMICOLON","var_declaration");
	$$->begin=$1->begin;
	$$->end=$7->end;
	...
	$7->parseTree.insert($7->type); 
	$7->parseTree.root->begin=$7->begin;
	$7->parseTree.root->end=$7->end;
	$7->parseTree.root->isLeaf=true;
	$6->parseTree.insert($6->type); 
	$6->parseTree.root->begin=$6->begin;
	$6->parseTree.root->end=$6->end;
	$6->parseTree.root->isLeaf=true;
	...
	terminalsChild4->data=$7->name;

    $3->parseTree.root->childNodes.push_back(terminalsChild1);
    $4->parseTree.root->childNodes.push_back(terminalsChild2);
    $6->parseTree.root->childNodes.push_back(terminalsChild3);
    $7->parseTree.root->childNodes.push_back(terminalsChild4);
...
|LOOP compound_statement WHILE LPAREN expression RPAREN SEMICOLON {}
;
...
| variable ASSIGNOP  logic_expression %prec LOWER_THAN_ELSE  { 
...
logic_expression_list : logic_expression_list COMMA logic_expression {
	$$=new SymbolInfo("logic_expression_list COMMA logic_expression","logic_expression_list");
	$$->begin=$1->begin;
	$$->end=$3->end;
	$$->parseTree.insert($$->type);
	$2->parseTree.insert($2->type); 
...
|logic_expression {
	$$=new SymbolInfo("logic_expression","logic_expression_list");
	$$->begin=$1->begin;
	$$->end=$1->end;
	$$->parseTree.insert($$->type);
